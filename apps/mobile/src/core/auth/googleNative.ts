...
export function configureGoogleNativeIfNeeded() {
  ...
  try {
    ...
    configured = true;
    console.log('[Auth] Google Sign-In native module successfully configured.');
  } catch (err) {
    console.warn('[Auth] Failed to configure Google Sign-In native module:', err);
  }
}

// Proactive initialization at module load time if native is enabled.
if (isGoogleNativeEnabled()) {
